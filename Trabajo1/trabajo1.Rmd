...
datos_t <- datos # Datos para entrenamiento, sin el 2018
...
```{r}
# Datos para entrenamientos
# Se crea la lista de valores para cada fila de 'datos_2018'
holidays_l_2018 <- apply(datos_2018,1,function(x){
  holiday <- subset(holidays, (year == x["PERIODO"] & day == x["DIA"] & month == x["MES"]), select=c(celebration,is_holiday))
...
# Se obtiene la transpuesta de la matrix, para ingresarla como dataframe
holidays_t_2018 <- t(holidays_l_2018)

holidays_df_2018 <- data.frame(holidays_t_2018)
colnames(holidays_df_2018) <- c("DIA_ESPECIAL", "DIA_FESTIVO", "CELEBRACION")

# Se agregan las columnas al DataFrame de 'datos_2018'
datos_2018 <- cbind(datos_2018, holidays_df_2018)

# En datos_2018 se encuentra nuestro set de datos de validación
```
