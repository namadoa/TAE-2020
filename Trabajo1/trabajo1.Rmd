...
diarioscomuna <- datos %>% group_by(COMUNA, DIAAÑO) %>% count()
diarioscomuna <- diarioscomuna %>% arrange(desc(n), COMUNA)
ggplot(diarioscomuna, aes(x = reorder(COMUNA, n), y = n, col = COMUNA)) + 
...
ggplot(semana, aes(x = reorder(SEMANA, Porcentaje), y = Porcentaje, 
  ...
  coord_flip() + 
  scale_fill_manual(values=mis.colores(20)) + 
  xlab("Semana") + 
  ylab("Porcentaje [%]") + 
...
```
```{r}
comuna <- write.csv(Comuna, file = "Comuna.csv", row.names = F)
barrio <- write.csv(barrio, file = "barrio.csv", row.names = F)
Mes <- write.csv(Mes, file = "Mes.csv", row.names = F)
semana <- write.csv(semana, file = "semana.csv", row.names = F)
Diasemana <- write.csv(Diasemana, file = "Diasemana.csv", row.names = F)
clase <- write.csv(clase, file = "clase.csv", row.names = F)
clasegravedad <- write.csv(clasegravedad, file = "clasegravedad.csv", 
                           row.names = F)
```
...
```{r}
Colors=c("lemonchiffon", "orange","red2")
...
datos_lm1 <- datos %>% group_by(FECHA, DIA_ESPECIAL, DIA_NOMBRE, DISENO) %>% count(name = "NRO_ACC") 
...
lm1_2018 <- datos_2018 %>% group_by(FECHA, DIA_ESPECIAL, DIA_NOMBRE, DISENO) %>% count(name = "NRO_ACC")
...
```
# El primer modelo tiene buenas métricas, pero tiene un problema al tener la variable DISENO, ya que no tenemos
# una forma fácil de obtener esta variable para la predicción

```{r}
# Modelo a partir del anterior teniendo en cuenta el DIA_FESTIVO y no DÍA ESPECIAL
datos_lm2 <- datos %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE, DISENO) %>% count(name = "NRO_ACC")
...
```
```{r}
lm2_2018 <- datos_2018 %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE, DISENO) %>% count(name = "NRO_ACC")
...
datos_lm3 <- datos %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE) %>% count(name = "NRO_ACC")
...
```{r}
lm3_2018 <- datos_2018 %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE) %>% count(name = "NRO_ACC")
...
```{r}
datos_lm4 <- datos %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE) %>% count(name = "NRO_ACC")
lm4 <- glm(NRO_ACC ~ DIA_FESTIVO+DIA_NOMBRE, family = "poisson", data = datos_lm4) # Modelo lineal generalizado, con familia poisson
summary(lm4)
```
```{r}
lm4_2018 <- datos_2018 %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE) %>% count(name = "NRO_ACC")
...
datos_lm5 <- datos %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE,CLASE) %>% count(name = "NRO_ACC")
...
```{r}
lm5_2018 <- datos_2018 %>% group_by(FECHA,DIA_FESTIVO, DIA_NOMBRE,CLASE) %>% count(name = "NRO_ACC")
