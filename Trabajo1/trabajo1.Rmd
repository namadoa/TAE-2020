...
```{r}
DIARIOS <- datos %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA,DIA, MES) %>% count(name = "NRO_ACC_DIA")
```

```{r}
SEMANAL <- datos %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA, SEMANA) %>% count(name = "NRO_ACC_SEMANA")
```

```{r}
MENSUAL <- datos %>% group_by(CLASE,BARRIO, GRAVEDAD, COMUNA, MES) %>% count(name = "NRO_ACC_MES")
```
...
datos_2018$DIA2 <- as.factor(day(datos_2018$FECHA))
```

```{r}
DIARIOS2018 <- datos_2018 %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA,DIA, MES) %>% count(name = "NRO_ACC_DIA")
```
