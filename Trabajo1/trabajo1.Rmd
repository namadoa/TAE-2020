...
## Clustering por latitud y longitud, k = 15. - Visualización previa de la clusterización por comunas.

```{r echo=FALSE, fig.align='center'}
knitr::include_graphics("Mapa.svg")
```

## Diagrama de barras - Clusters por numero de puntos.

```{r echo=FALSE, fig.align='center'}
knitr::include_graphics("Clusters.svg")
```

## Función para el calculo de distancias de datos geoespaciales, haciendo uso de la libería geosphere
```{r}
...
## Creación nuevo data frame para clusering
```{r}
...
## Mapa de clusterización según latitud y longitud.
```{r}
...
## Mapa de clusterización teniendo en cuenta no latitud y longitud, sino numero de accidentes totales según barrio.
...
df <- as.matrix(table(datos_t$BARRIO, datos_t$GRAVEDAD))  #Numero deaccidentes por barrio segun categoría 
df <- data.frame(HERIDO = df[,1], MUERTO = df[,2], SOLO_DAÑOS = df[,3])
df
```
## Clustering haciendo uso del algoritmo - k means - busqueda del numero optimo de clusters.


```{r}
#Una vez tenemos los datos preprocesados y el subset de datos seleccionados, procedemos a realizar un scalamiento y centrado del dataset.
...
```{r}
#En un principio seleccionaremos un numero aleatorio de clusters y observaremos como son creados, empezaremos con  k = 3.

kmm = kmeans(scaled_data, 3, nstart = 50, iter.max = 15 )
kmm
```
# En este caso debemos corroborar el valor (between_SS/total_SS), el cual es ~77%. Lo que realmente representa esta proporción es la relación entre la cantidad de la suma de los cuadrados totales de los data points que estan en los clusters. Nosotros queremos aumentar este valor a medida que aumentamos el numero de clusters, sin embargo, tampoco queremos llegar al overfitting, por ejemplo si usaramos un k igual al numero de barrios, tendría serios problemas de sobreajuste.
...
```{r}
## Encontremos un valor de k para el cual el modelo no se sobreajuste y al mismo tiempo agrupe los datos según la distribución real.

## Este metodo considera el porcentaje de varianza explicada en función del número de grupos (clusters).

## Lo ideal es elegir un número de clusters de tal forma que al agregar uno más, no se obtenga un mejor modelado de los datos.

set.seed(123)
#Vamos a calcular y gráficar WSS(total within - cluster sum of square) para k = 2 to k = 10
...
# En este caso, para k = 4 el between_ss/total_ss ratio tiende a cambiar lentamente y a ser menos cambiante en comparación con otros k, como 1 o 2., k = 5 también parece ser un candidato potencial.


# ¿Cómo decidimos cuál será la elección óptima?
```{r}
...
plot(d_clust)
```
# En este caso, los 3 mejores modelos son para un  k = (7, 8, 12).
...
```{r}
#Método del codo.
fviz_nbclust(scaled_d, kmeans, method = "wss") +
...
```
## Generación de clusterización según los k optimos 
```{r}
# Según los diferentes métodos k = 4, parece ser muy optimo para la generación de la clusterización.
#Generación de clusterización con k = 4.
...
library(cluster)
dis = dist(df)^2
res = pam(dis, 2)  #Según el método de la silueta el mejor k es 2, sin embargo, 3 y 4 arrojan buenos resultados.
