...
setwd("e:/Users/camil/Escritorio/TAE/Trabajo 1")
library(stringr)
library(dplyr)
library(lubridate)
library(ggplot2)
...
```{r}
datos_2014 <- read.csv("Incidentes_georreferenciados_2014.csv", 
                       sep = ",", encoding = "UTF-8")
datos_2015 <- read.csv("Incidentes_georreferenciados_2015.csv", 
                       header = T, sep = ",", encoding = "UTF-8")
datos_2016 <- read.csv("Incidentes_georreferenciados__2016.csv", 
                       header = T, sep = ",", encoding = "UTF-8")
datos_2017 <- read.csv("Incidentes_georreferenciados_2017.csv", 
                       header = T, sep = ",", encoding = "UTF-8")
datos_2018 <- read.csv("Incidentes_georreferenciados_2018.csv", 
                       header = T, sep = ",", encoding = "UTF-8")
holidays <- read.csv("holidays.csv", header = T, sep=",", encoding = "UTF-8")
...
datos$DIA2 <- as.factor(day(datos$FECHA))
...
```{r}
DIARIOS <- datos %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA,NUMEROCOMUNA, DIAAÑO, MES) %>% count(name = "NRO_ACC_DIA")

DIASEMANA <- datos %>% group_by(CLASE, BARRIO, GRAVEDAD, COMUNA, NUMEROCOMUNA,
                                DIA_NOMBRE, MES) %>% count(name = "NRI_ACC_DIANOMBRE")
```

```{r}
SEMANAL <- datos %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA,NUMEROCOMUNA, SEMANA) %>% count(name = "NRO_ACC_SEMANA")
```

```{r}
MENSUAL <- datos %>% group_by(CLASE,BARRIO, GRAVEDAD, COMUNA,NUMEROCOMUNA, MES) %>% count(name = "NRO_ACC_MES")
```
...
```{r}
#CREACIÓN VARIABLE SEMANA 2018
datos_2018$SEMANA <- as.factor(week(datos_2018$FECHA))

#CREACIÓN DE LA VARIABLE DIA DEL AÑO
datos_2018$DIAAÑO <- lubridate::yday(datos_2018$FECHA)
```

```{r}
DIARIOS2018 <- datos_2018 %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA,DIAAÑO, MES) %>% count(name = "NRO_ACC_DIA")
```

```{r}
SEMANAL2018 <- datos_2018 %>% group_by(CLASE,BARRIO,GRAVEDAD, COMUNA, SEMANA) %>% count(name = "NRO_ACC_SEMANA")
```

```{r}
MENSUAL2018 <- datos_2018 %>% group_by(CLASE,BARRIO, GRAVEDAD, COMUNA, MES) %>% count(name = "NRO_ACC_MES")
```
